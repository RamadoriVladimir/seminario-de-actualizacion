...
void manageUsers(RoleStrategy* roleStrategy);
void listUsers();
void deleteUser(const std::string& currentUser, RoleStrategy* roleStrategy);
void changeUserRole(const std::string& currentUser, RoleStrategy* roleStrategy);
...
int main() {
    ...
    while (true) {
        ...
        case 2:
            {
                std::cout << "La creación de cuentas está restringida. Por favor, inicie sesión como administrador." << std::endl;
                break;
            }
        case 3:
...
void showMenu(const std::string& username, RoleStrategy* roleStrategy) {
    ...
    do {
        ...
        if (roleStrategy->canManageUsers()) {
...
void manageUsers(RoleStrategy* roleStrategy) {
    if (!roleStrategy->canManageUsers()) {
        std::cout << "No tiene permisos para gestionar usuarios." << std::endl;
        return;
    }

    int opcion;
    do {
        std::cout << "\nGestion de Usuarios:" << std::endl;
        std::cout << "1) Listar usuarios" << std::endl;
        std::cout << "2) Crear nuevo usuario" << std::endl;
        std::cout << "3) Eliminar usuario" << std::endl;
        std::cout << "4) Cambiar rol de usuario" << std::endl;
        ...
        case 2:
            ...
            break;
        case 3:
            deleteUser("", roleStrategy); 
            break;
        case 4:
            changeUserRole("", roleStrategy);
            break;
        case 5:
            return;
        default:
...
void listUsers() {
    ...
    for (const auto &user : users) {
        std::cout << "Usuario: " << user.first 
                  << " - Rol: " << user.second.second->getRoleName() << std::endl;
    }
}

void deleteUser(const std::string& currentUser, RoleStrategy* roleStrategy) {
    if (!roleStrategy->canManageUsers()) {
        std::cout << "No tiene permisos para eliminar usuarios." << std::endl;
        return;
    }
    ...
    if (username == currentUser) {
        std::cout << "No puede eliminarse a sí mismo." << std::endl;
        return;
    }

    users.erase(username);
    std::cout << "Usuario eliminado con exito." << std::endl;
}

void changeUserRole(const std::string& currentUser, RoleStrategy* roleStrategy) {
    if (!roleStrategy->canManageUsers()) {
        std::cout << "No tiene permisos para cambiar roles de usuario." << std::endl;
        return;
    }

    std::string username, newRole;
    ...
    if (username == currentUser) {
        std::cout << "No puede cambiar su propio rol." << std::endl;
        return;
    }

    std::cout << "Ingrese el nuevo rol (Administrador, Cliente, Trabajador de deposito): ";
    std::getline(std::cin, newRole);

    if (newRole == "Administrador") {
        users[username].second = std::make_unique<AdminStrategy>();
    } else if (newRole == "Cliente") {
        users[username].second = std::make_unique<ClientStrategy>();
    } else if (newRole == "Trabajador de deposito") {
        users[username].second = std::make_unique<WarehouseWorkerStrategy>();
    } else {
        std::cout << "Rol no valido. No se realizaron cambios." << std::endl;
        return;
    }

    std::cout << "Rol de usuario actualizado con exito." << std::endl;
}
...
void createAccount() {
    ...
    std::cout << "Ingrese el rol (Administrador, Cliente, Trabajador de deposito): ";
