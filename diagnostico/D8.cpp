...
std::unordered_map<std::string, std::pair<std::string, std::string>> users = {
    {"juan", {"Juan123._", "Administrador"}},
    {"enzo", {"Enzo456!.", "Vendedor"}},
    {"maria", {"Maria789.!", "Cliente"}},
    {"admin", {"Admin123_!", "Administrador"}},
    {"empleado", {"Empleado456!.", "Trabajador de deposito"}},
};
...
void showMenu(const std::string& username, const std::string& role);
...
int main()
{
    while (true)
    {
        ...
        case 1:
            ...
            {
                showMenu(username, users[username].second);  
            }
            break;
        case 2:
            createAccount();
...
void createAccount()
{
    ...
    users[newUsername] = {newPassword, role};
